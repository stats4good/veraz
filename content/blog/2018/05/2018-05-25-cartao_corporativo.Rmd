---
title: "Análise dos Gastos do governo usando o Cartão Corporativo"
author: "Victor Schmidt Comitti"
date: 2018-05-25
slug: cartao_corporativo
banner: "img/banners/banner_ccorp.jpg"
summary: "Entendendo a trajetória dos gastos do governo com o Cartão Corporativo."
categories:
  - Economia
  - Cartão corporativo
  - CPGF
...
```{r echo = FALSE}
autoplot(somavalores.def/1000000, ts.colour = 'red', main="Evolução dos gastos com Cartão Corporativo do Governo", ylab="R$ Milhões") + 
  theme_bw()
```
...
```{r echo = FALSE}
autoplot(decomposeval, main="Decomposição da série de gastos totais com o Cartão Corporativo") +
  theme_bw() +
...
```{r echo = FALSE}
autoplot(proclas, ts.colour = 'black', main="Evolução da proporção de gastos secretos com Cartão Corporativo do Governo") +
  theme_bw()
```
...
```{r echo = FALSE}
autoplot(decomposeprop, main="Decomposição da série de gastos secretos") +
  theme_bw() +
  theme(strip.background = element_rect(fill = 'transparent'))
