---
title: 'Visualizando os dados Programa Bolsa Família'
...
As condições para ser uma família eletiva ao recebimento da ajuda financeira é definida como aquelas com renda per capita mensal entre R\$ 85,00 e R\$ 170,00 que tenham em sua composição gestantes e crianças ou adolescentes entre 0 e 17 anos e extremamente pobres (com renda per capita até R$ 85,00). Além disso, para se candidatar ao programa, é necessário que a família esteja inscrita no Cadastro Único para Programas Sociais do Governo Federal, com seus dados atualizados há menos de 2 anos[^1]. 
...
Nos gráficos abaixo podemos visualizar a série histótica do número de beneficiários ao longo dos anos para o mês de janeiro. O gráfico do lado esquerdo apresenta o número absoluto de beneficiários, enquanto o o grafico da direira apresenta a taxa de beneficiários, ou seja, o número de beneciários dividido pela população da região em análise. Notamos que as rigões Norte e Nordeste são as que possuem os maiores taxas de beneficiários, ficando acima de 7,5% da população.
...
Com intuito de comparar a série de histórica do número de beneficiários da região com ela mesma propomos o gráfico abaixo com a escala do eixo Y livre. Analisando cada região isoladamento é possível visualizar que o número de benficiários à partir de 2014 e 2015 vem caindo, exceto para região Norte que tem um crescimento praticamente linear.
...
Para corroborar o resultado visual foi realizado o cálculo do Índice de Moran através da função da `moran.mc` do pacote `spadep`. O valor encontrado foi `r round(sim1$statistic, 3)` com p-valor igual a `r sim1$p.value`, ou seja, rejeitamos a hipótese de independência espacial e portanto temos indicativos de correlação espacial. 

## Considerações

Como podemos ver o Portal da Transparência disponiliza dados de diversas fontes que nos permite analisar e enteder melhor as diferenças do nosso país. Na postagem de hoje foi possível verificar uma grande redução do número de beneficiários ao longo dos anos, quebrando uma tendência de crescimento. Os motivos que levaram esta redução não foi analisada neste primeiro momento, mas é já é sabido que huveram muitos cortes à partir de 2015 em diversas frentes.
